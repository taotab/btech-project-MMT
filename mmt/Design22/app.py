...
def get_coordinates_db(conn, point_id):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT latitude, longitude FROM gps_data WHERE id=?", (point_id,))
    row = cursor.fetchone()
    if row is not None:
        return row[0], row[1]
    else:
        print(f"ID {point_id} not found in the dataset.")
        return None, None
...
def calculate_distance():
    ...
    if request.method == 'POST':
        ...
        datetime1 = datetime.strptime(date1 + ' ' + time1, '%Y-%m-%d %H:%M')
        datetime2 = datetime.strptime(date2 + ' ' + time2, '%Y-%m-%d %H:%M')
